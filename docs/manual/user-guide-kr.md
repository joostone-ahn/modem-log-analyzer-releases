...
| `.dlf` | Qualcomm DLF 로그 |
| `.sdm` | Samsung Shannon DM 로그 |

### 3.3 멀티 파일 선택 (SDM)

여러 `.sdm` 파일을 동시에 선택하면 시간순으로 병합됩니다.

- **Open File** 다이얼로그에서 Cmd/Ctrl+클릭으로 여러 `.sdm` 파일 선택
- **드래그 앤 드롭**으로 여러 `.sdm` 파일을 동시에 드롭
- 병합 결과가 단일 pcap으로 변환되어 메시지 목록에 표시됩니다

### 3.4 파일명 상태 표시

파일 분석 후 상단에 파일명이 표시됩니다.

- 단일 파일: `📄 TC1.1.sdm`
- 멀티 파일 병합: `📄 8 files merged`

### 3.5 Save (PCAP 다운로드)

파일 파싱 완료 후 상태 표시 우측에 **Save** 버튼이 나타납니다.
...
상단 필터 입력란에 텍스트를 입력하면 프로토콜, 채널, 메시지 이름 기준으로 실시간 필터링됩니다.

### 4.4 상세 패널

메시지를 클릭하면 우측에 전체 디코딩 트리가 표시됩니다.
- **⌘+F / Ctrl+F**: 상세 내용 내 검색 포커스
- 검색어 입력 시 첫 번째 매치로 자동 이동
- **Enter**: 다음 검색 결과로 이동
