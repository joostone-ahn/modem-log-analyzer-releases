...
**Disconnect** 버튼을 클릭하면 캡처가 중지되고 USB 디바이스가 Windows로 반환됩니다.

### 2.6 단말 교체

1. **Disconnect** 클릭 → USB가 Windows로 반환됨 (장치 관리자에서 포트 확인)
2. 기존 단말을 분리하고 새 단말 연결
3. 장치 관리자에서 새 단말의 포트가 잡히는지 확인
4. **Connect** 클릭 → 새 단말로 캡처 시작

> 앱 재시작 없이 단말 교체가 가능합니다.
...
> ⚠️ **Windows (EXE) 환경에서는 현재 이 기능이 비활성화**되어 있습니다. 기술적 제한으로 인해 AT 명령 실행 후 캡처 성능이 저하되는 문제가 있어 향후 업데이트에서 해결 예정입니다.  
> macOS에서는 정상 동작합니다.
...
### Windows (EXE)
브라우저 탭을 닫고, 작업 관리자에서 `DM-Viewer` 프로세스를 종료합니다.  
다음 실행 시 이전 프로세스가 자동으로 정리됩니다.

### Windows (소스 실행, run-wsl.bat)
...
| v1.1.0 | 2026-06-05 | 단말 교체 지원, AT 명령 WSL 비활성화, launcher wsl --shutdown 기반 재시작 |
