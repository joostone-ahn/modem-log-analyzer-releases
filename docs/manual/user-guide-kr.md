...
> **이후 실행:** WSL 초기화 후 서버를 시작합니다 (약 10초).  
> **재실행 시:** 이전 실행이 남아있으면 WSL을 리셋합니다. "Resetting WSL..." 단계에서 10~15초 소요될 수 있습니다.

⚠️ 최초 실행 시 EXE 옆에 `dm-viewer-linux` 파일이 생성됩니다. 이 파일을 삭제하지 마세요.

### macOS (Private 배포)

macOS 버전(ZIP)은 별도 요청을 통해 제공됩니다.

> **Author:** JUSEOK AHN (ajs3013@lguplus.co.kr)

ZIP 파일 수령 후:
1. 압축을 해제합니다
2. `run/run.command`를 더블클릭합니다 (최초 실행 시 venv + 의존성 자동 설치)
