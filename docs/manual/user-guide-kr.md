...
브라우저 우측 상단의 **⏻ (종료) 버튼**을 클릭합니다.

1. "Shutdown the server and close the application?" 확인 다이얼로그가 표시됩니다
2. **Shutdown**을 클릭하면:
   - 캡처 중이면 자동 중지
   - USB 디바이스가 Windows로 반환됨 (detach + unbind)
   - 서버가 종료됨
3. 브라우저 상태 표시가 빨간 ● "Server stopped"로 변경됩니다
4. 기존 캡처 데이터는 브라우저에서 계속 확인 가능합니다

> **참고:** 종료 버튼을 사용하지 않고 작업 관리자에서 프로세스를 종료해도 됩니다. 브라우저는 5초 내에 서버 종료를 감지하여 동일하게 "Server stopped" 상태로 전환됩니다.

> **재시작:** 종료 후 다시 사용하려면 EXE를 다시 실행하세요.
